...
CREATE TABLE IF NOT EXISTS customers (	-- use to store information about the people buying the product
	...
	email VARCHAR(50) NOT NULL UNIQUE,	-- UNIQUE constraint used because email address are always unique
	address VARCHAR(25) NOT NULL UNIQUE,	-- contains the unit number and street (UNIQUE used because the number for a street or a unit number should be unique)
	city VARCHAR(20) NOT NULL,	-- no UNIQUE constraint because name of cities is not always unique
...
CREATE TABLE IF NOT EXISTS products (	-- used to store information about the products that the company offers and who ordered it
	...
	inStock INT NOT NULL,	-- represents how much of a product was in stock before more were purchased (based on the info in the products.php file and the form.js file)
...
CREATE TABLE IF NOT EXISTS services (	-- used to store information about the services that the company sells and who ordered it
	...
	discServ DEC(2,2)	-- represents the discount the service has (based on the info in products.php and the form.js file)	
) ENGINE = innoDB;
