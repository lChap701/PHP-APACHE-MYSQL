					...
					<?php
						...
						function validEmail(String $email) {
							...
							if (empty($email)) {
								...
								$errCtr++;
							} elseif (!preg_match("/.*([@]).*/", $email)) {	// checks if email does or does not contain an @ symbol
						...
						function validAddr(String $ad) {
							...
							if (empty($ad)) {
								...
								$errCtr++;
							} elseif (!preg_match ("/^(\d{1,3} (.*([A-Z|a-z]).*))/", $ad)) {
						...
						function validCredexpm(String $expm) {
							...
							if (empty($expm)) {
								...
								if ($currMon > $monNum && $currYear >= $expy) {
									...
									echo "<script>alert('". var_dump($expy)."');</script>";
									$expm = "";
									$errCtr++;
								}
							}
								...
								if (ctype_digit($cvv)) {
									if (strlen($cvv) < 10) {
										if ((int) $cvv < 100) {
											...
											echo "<script>alert('$cvv is invalid, CVV should not be greater than 999');</script>";
											$cvv = "";
											$errCtr++;
										}
									} else {
							...
							<div class="col-50">
								...
								<div class="row">
									...
									<div class="col-25">
										...
										<select id="ctry" name="ctry" onchange="ctrySelected();">
											<option value=""></option>
											<!-- PHP generated code -->
											<?php
												/* Used to simplify code */
												/* Prints all the options for ctry */
