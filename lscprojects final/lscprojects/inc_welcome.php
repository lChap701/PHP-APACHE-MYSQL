<?php
	...
	/* Updated 8/9/2020 by Lucas Chapman */
	...
	$password = null;
	$errCtr = 0;
	$_SESSION['please_login'] = "<script>alert('Please consider logging in. Note: click on \"Remember Me\" checkbox to have your username remembered');</script>\n";

	// Checks if there is no values in $_SESSION['login']
	if (!isset($_SESSION['login'])) {
		$_SESSION['login'] = false;
	}
	
	// Checks if the logout message should be displayed 
	if (isset($_SESSION['logout'])) {
		echo "<script>alert('You are now logged out');</script>";	// displays a logout message
		unset($_SESSION['logout']);	// clears values in $_SESSION['logout']
	}
	...
	<?php
		...
		function login() {
			...
			if ($_SESSION['login']) {	// checks if the user is logged in
				...
				if ($errCtr == 0 && !isset($_SESSION['message_displayed'])) {
					echo $_SESSION['please_login'];	// displays a login message
					$_SESSION['message_displayed'] = true;	// keeps the login message from being displayed more than once 
				}
			}
		}
	?>
