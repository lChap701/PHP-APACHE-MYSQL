...
<!-- This PHP webpage demonstrates forms and form handling, as well as superglobal variables, database proccessing, sessions, and OOP -->
<!DOCTYPE html>	
<html lang="en">
	<head>
		<meta charset="utf-8">
					...
					<?php
						...
						if (isset($_POST['purchase'])) {
							...
							if ($errCtr > 0) {
								...
								/* Messages */
								echo "<p>Thank you $fname $lname, your purchase is being processed</p>\n";
								echo "<p>Click on 'Checkout' to have an email with your order ID sent to $email</p>\n";
								...
								echo "<a href='checkout.php' class='checkout' title='Checkout'><b>Checkout</b></a>\n";
						...
						/*
						 * Checks the data that was entered is contained in the Products table
						 */
						function findProd(String $prod) {
							global $con, $errCtr;
							
							$result = $con -> query("SELECT DISTINCT prodName FROM products WHERE prodName = '" . $prod . "'");
						...
						/*
						 * Checks the data that was entered is contained in the Services table
						 */
						function findServ(String $serv) {
							global $con, $errCtr;
							
							$result = $con -> query("SELECT DISTINCT servName FROM services WHERE servName = '" . $serv . "'");
							if ($result -> num_rows == 0) {
						 ...
						 */
						function createOrdObj() {
							global $fname, $lname, $email, $addr, $city, $st, $ctry, $zip, $crednum, $credexpm, $credexpy, $cvv, $item, $prods, $qty, $servs, $mons, $mem, $cost, $disc;

							$ord = new Orders();	// creates an object with default data
							// Adds the data that was entered to the object
							$ord -> setFname($fname);
							$ord -> setLname($lname);
							$ord -> setEmail($email);
							$ord -> setAddr($addr);
							...
							// Checks if a state was selected and determines what it should be set to
							if (!empty($st)) {
								$ord -> setState($st);
							}
							...
							$ord -> setCntry($ctry);
							$ord -> setZip($zip);
							$ord -> setCcnum($crednum);
							$ord -> setCcexpm($credexpm);
							$ord -> setCcexpy($credexpy);
							$ord -> setCvv($cvv);
							$ord -> setItemType($item);

							// Checks what item was selected for the "Type Of Item" radio button
							if ($item == "prod") {
								$ord -> setItem($prods);
								$ord -> setQtyMon($qty);
							} elseif ($item == "serv") {
							...
							// Checks if $mem doesn't equal "on"
							if ($mem != "on") {
								$mem = "off";
							}

							$ord -> setMem($mem);
							$ord -> setCost(str_replace("$", "", str_replace(",", "", $cost)));	// help makes $cost a float rather than a String
							$ord -> setDisc($disc);

							echo "<script>console.log('An order object was created!')</script>\n";
							$_SESSION['order'] = $ord;	// puts the object in session storage
						}
