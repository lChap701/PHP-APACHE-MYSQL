...
<!-- This code is part of a separate PHP file that is used to display to the user that they have logged in and demonstrates the use of form handling, superglobal variables, and cookies and sessions -->
<?php
	...
	$_SESSION['login'] = false;	// used to determine invalid logins and display error messages
	$_SESSION['create'] = false;	// used to determine invalid create login attempts and display error messages
	...
	// Determines if a cookie should be created if a valid username has been entered or created and the 'Remember Me' checkbox was checked
	if ($_SESSION['login']) {
		if (isset($_POST['remember'])) {
			setcookie("unme", $uname, time() + 60 * 60 * 24 * 30);	// creates a cookie for the username that was entered that will expire 30 days from the current date
		} else {
			// Deletes a cookie when a 'unme' cookie exists and the 'Remember Me' checkbox is not checked 
			if (isset($_COOKIE['unme'])) {
				setcookie("unme", $uname, time() - 60 * 60 * 24 * 30);	// deletes an existing cookie for the username that was entered 
			}
		}
	} elseif ($_SESSION['create']) {
		setcookie("unme", $uname, time() + 60 * 60 * 24 * 30);	// creates a cookie for the username that was entered 	
	}
?>	
<!DOCTYPE html>
<html lang="en">
			...
			<div id="content">
				<?php
					...
					function checkUsername(String $uname) {
						...
						return str_replace(" ","", $uname);	// used to remove all whitespace characters
					}
					...
					/* 
					 * Reads the password.txt file and calls other functions
					 */
					function readRegFile() {
						...
						if (file_exists($pswFile)) {	// checks if the password text file is available
							$arrData = file($pswFile);	// reads all the data in the password.txt file and stores it in an array
							
							// The length of the non-encrypted users array
							$usersCnt = count($arrData);	// used for validation in the readEncFile() function
							...
							if (isset($_POST['create'])) {	// checks if the 'Create Login' button was pressed
								for ($i = 0; $i < count($arrData); $i += 2) {
									if (trim($arrData[$i]) == $uname) {	// if the $uname matches with any username in the file
										$written = true;
										writeRegFile($pswFile, $written);
										$i = count($arrData);	// sets $i to the length of the array to end the loop
									} elseif ($i == count($arrData) - 2) {	// if no matches were found	
										writeRegFile($pswFile, $written);
										addToEncFile();
									}	
								}
								...
								for ($i = 0; $i < count($arrData); $i += 2) {
									readEncFile(trim($arrData[$i]), trim($arrData[$i + 1]), $usersCnt, $check);	// called to encrypt data and write that data in the enc_password.txt

									if (trim($arrData[$i]) == $uname && trim($arrData[$i + 1]) == $psw) {	// if the $uname and $psw match with any of the data in the file
										$check = true;
										readEncFile(trim($arrData[$i]), trim($arrData[$i + 1]), $usersCnt, $check);	// called to perform validation
										$i = count($arrData);	// set to the length of the arrays to end to the loop
									} elseif ($i == count($arrData) - 2) {	// checks if no matches that were found
					...
					/*
					 * Writes in the regular text file if the form was submitted by pressing the 'Create Login' button
					 */
					function writeRegFile(String $file, bool $taken) {
						global $uname, $psw;
						$fp = fopen($file, "a");	// 'a' sets the mode for writing only, starts at the beginning of the file and adds new data to the file without overwriting any existing data
					...
					/*
					 * Writes in the encrypted text file if the form was submitted by pressing the 'Create Login' button
					 */
					...
					/*
					 * Displays a message back to the user based on $_SESSION['login']
					 */
					function searchPasswordFile() {
						global $uname, $psw;
						
						// Checks if the user is logged in
						if ($_SESSION['login']) {	// if the user is logged in
							echo "<script>alert('$uname and $psw matches a known user\'s username and password');</script>\n";
							echo "<p>Welcome $uname!</p>\n";
							echo "<a title='Go Back' href='loginform.php'>Go Back</a>\n";
						} else {
							echo "<script>alert('$uname and $psw does not match any known user\'s username and password, consider creating a new login');</script>\n";
							echo "<script>document.getElementById('content').classList.add('expand');</script>\n";
							echo "<a title='Go Back' href='loginform.php'>Go Back</a>\n";
						}
					}
