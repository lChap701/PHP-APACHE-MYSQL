			...
			<div id="container">
				<section class="sect-2">
					...
					<?php
						...
						/*
						 * Checks data that was entered to determine if they should be added to Customers table using prepare
						 */
						...
						/*
						 * Checks data that was entered to determine if they should be added to Orders table using prepare
						 */
						function insertOrdRow() {
							...
							if ($id > 0 && $arrEncData != null) {
								...
								if ($con -> error) {
									...
									// Bind
									$prep -> bind_param("ississ", /* $ordId, */ $custId, $ccnum, $expm, $expy, $cv, $memDisc);	// binds all parameters to placeholders
									if ($prep -> error) {
										...
										// Set values to parameters
										//$ordId = $id;	// the Order ID is the same as the Customer ID for this table
										$custId = $id;
										$ccnum = $arrEncData[0];	
										$expm = $credexpm;
										$expy = $credexpy;
						...
						/*  
						 * Checks the hash of credit card information to check if a row has already been added with the data that was entered
						 */
						function checkHash() {
						...
						function checkFkOrdId() {
							...
							$result = $con -> query("SELECT ordId FROM orders NATURAL JOIN customers WHERE email = '$email' AND address = '$addr' AND custId = $custId");
